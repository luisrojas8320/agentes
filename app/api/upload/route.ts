...
// Se elimina la importación de TextLoader para evitar el bug de build en Vercel
...
/**
 * Realiza OCR en un archivo de imagen utilizando la API de OCR.space.
 */
...
async function processAndSplitDocument(file: File): Promise<string[]> {
  ...
  if (file.type.includes('pdf')) {
    ...
    // Si el PDF tiene poco texto, podría ser una imagen escaneada. Intentamos OCR.
    if (pdfText.length < 100) {
      ...
      try {
        ...
        rawText = pdfText; // Usamos el texto extraído aunque sea corto
      }
    } else {
      rawText = pdfText;
    }
  } else {
    // WORKAROUND: Reemplazamos TextLoader con una lectura de texto nativa y directa
 ...
 * y guardar los trozos en la base de datos.
 */
